<script lang="ts">
	...
	const joinMatch = async () => {
		if (form.reportValidity()) {
			await goto(
				`/game/${roomID}?x-name=${$page.url.searchParams.get('x-name')}&o-name=${playerName}`
			);
		}
	};
</script>
...
<div class="w-full h-full flex flex-col justify-center items-center">
	...
	<form bind:this={form} action="" on:submit|preventDefault={joinMatch}>
		<PlayerNameInput playerType="O" bind:value={playerName} />
